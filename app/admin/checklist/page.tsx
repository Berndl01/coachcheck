...
import { isPersistentRateLimit } from '@/lib/utils/rate-limit';
...
export default async function AdminChecklistPage() {
  ...
  // Admin-Check: DB-gestützt (admin_roles) mit ADMIN_EMAILS-Fallback.
  ...
  checks.push({
    label: 'Report-Modell',
    status: 'ok',
    detail: process.env.ANTHROPIC_REPORT_MODEL ?? 'claude-opus-4-7 (Code-Fallback) — Empfehlung: claude-opus-4-8 setzen',
  });

  // 4b. Rate-Limit & Bot-Schutz (für Public Sale relevant)
  checks.push({
    label: 'Persistentes Rate-Limit (Upstash)',
    status: isPersistentRateLimit() ? 'ok' : 'warn',
    detail: isPersistentRateLimit()
      ? '✓ aktiv (Upstash Redis)'
      : '🟡 nur In-Memory-Fallback (pro Serverless-Instanz isoliert) — für Public Sale UPSTASH_REDIS_REST_URL/TOKEN setzen',
  });
  checks.push({
    label: 'Bot-Schutz Kontaktformular (Turnstile)',
    status: process.env.TURNSTILE_SECRET_KEY && process.env.NEXT_PUBLIC_TURNSTILE_SITE_KEY ? 'ok' : 'warn',
    detail: process.env.TURNSTILE_SECRET_KEY && process.env.NEXT_PUBLIC_TURNSTILE_SITE_KEY
      ? '✓ aktiv'
      : '🟡 inaktiv — für Public Sale TURNSTILE_SECRET_KEY + NEXT_PUBLIC_TURNSTILE_SITE_KEY setzen',
  });
      ...
      <main className="max-w-4xl mx-auto px-4 md:px-8 py-16">
        ...
        <div className="space-y-4">
          ...
          <div className="p-5 bg-bone-soft rounded-md border border-bone-line">
            ...
            <p className="text-sm">
              Schnelltest (19€) kaufen → Checkout durchführen → Assessment starten → PDF-Report generieren.
