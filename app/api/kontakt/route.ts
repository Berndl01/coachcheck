...
export async function POST(request: NextRequest) {
  ...
  const { name, email, phone, club, plan, message, website, turnstileToken } = body ?? {};

  // (1) Honeypot: echtes Feld, für Menschen unsichtbar. Gefüllt = Bot →
  //     wir tun so, als wäre alles ok, senden aber nichts.
  if (typeof website === 'string' && website.trim() !== '') {
    return NextResponse.json({ ok: true });
  }

  // (2) Rate-Limit pro IP (+ pro E-Mail), gegen Massen-Spam.
  const ip = request.headers.get('x-forwarded-for')?.split(',')[0]?.trim() || 'unknown';
  const rl = await checkRateLimit(`kontakt:ip:${ip}`, 5, 600_000);
  if (!rl.ok) {
  ...
  // (3) Cloudflare Turnstile (nur aktiv, wenn TURNSTILE_SECRET_KEY gesetzt).
